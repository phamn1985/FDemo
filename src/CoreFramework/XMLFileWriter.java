...
public class XMLFileWriter {
	public static XmlSuite createXMLTestSuite(String parrallel) {
		...
		String uidCol = "DEVICE UID";
		String devNameCol = "DEVICE NAME";
		String androidVersionCol = "ANDROID VERSION";
		...
		try {
			...
			for (int i = 0; i < columnNames.size(); i++) {
				...
				if (columnNames.get(i).getStringCellValue().trim().equalsIgnoreCase(uidCol)) {
					uidColInd = col.getColumnIndex();
				...
				} else if (columnNames.get(i).getStringCellValue().trim().equalsIgnoreCase(devNameCol)) {
					devNameColInd = col.getColumnIndex();
				} else if (columnNames.get(i).getStringCellValue().trim().equalsIgnoreCase(androidVersionCol)) {
					androidVerColInd = col.getColumnIndex();
				}
			...
			for (int x = 1; x < alltestplanRows.size(); x++) {
				...
				if (status.trim().equalsIgnoreCase("YES")) {
					...
					cursorCell = (XSSFCell) columnValue.get(devNameColInd);
					test.addParameter("deviceName", cursorCell.getStringCellValue().trim());

					cursorCell = (XSSFCell) columnValue.get(uidColInd);
					test.addParameter("deviceUid", cursorCell.getStringCellValue().trim());

					cursorCell = (XSSFCell) columnValue.get(androidVersionCol);
					test.addParameter("androidVersion", cursorCell.getStringCellValue().trim());
